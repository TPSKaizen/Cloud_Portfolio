...
curl https://raw.githubusercontent.com/SonarSource/docker-sonarqube/refs/heads/master/example-compose-files/sq-with-postgres/docker-compose.yml > docker-compose.yml

# Change to myagent directory
cd myagent

# Add user to test command in docker compose file
sudo sed -i 's/test: \["CMD-SHELL", "pg_isready"\]/test: ["CMD-SHELL", "pg_isready -U sonar"]/g' docker-compose.yml

# Update VM max memory and file size maximum for SonarQube requirements
